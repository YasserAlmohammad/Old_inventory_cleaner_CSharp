...
namespace InvCleaner
{
    class Program
    {
        ...
        {
            //read the target process name from a file
            ...
            readItemIDs(); //load a list of item ids and their names
            //process id comes in args
           //  targetProcess = Process.GetProcessById(Int32.Parse(args[0]));
            Process [] processList=Process.GetProcessesByName(processName);
            if(processList.Length==0){
                Console.WriteLine("no porcess running with the name"+processName);
                return;
            }
            targetProcess = processList[0];

            /**
                         * typically: item ids of wanted items are stored in file wantedIds.txt in the same dir as exe
                         * we will use a mixture of pixel and memory reading combination to determine what to delete(memory)
                         * and how to delete it(pixel)
                         * using 1024x768 windowed mode 
                         */


            //press I button to open inventory window
     ...
     //       PostMessage(targetProcess.MainWindowHandle, WM_UP, vk, (scanCode));
            ...
            //press I button to close inventory window
     //       PostMessage(targetProcess.MainWindowHandle, WM_KEYDOWN, vk, scanCode);
     //       PostMessage(targetProcess.MainWindowHandle, WM_UP, vk, (scanCode));
